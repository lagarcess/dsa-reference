...
test_cases = [
    ...
    (LinkedList([1,2-1,3-1,2,3,5-2,5-2,4,5,7-2]), LinkedList([1,2,3,4,5])),
    (LinkedList([1,2-1,3-1,4-1,5-1,6-1,7-1]), LinkedList([1,2,3,4,5,6])),
]
...
def test_remove_duplicates(solution_func, linked_list, expected_output):
    ...
    assert actual == expected, f"Expected {expected}, but got {actual} for {solution_func.__name__}"